...
def run_SciPyOp(obj,   threads=1,  rtg=[True,True,False],  kernel_id=-1,  save_means=False, fileoutput=False, save_sampler=False, **kwargs):      
    ...
    if (obj.mod_dynamical):
        ...
        mod='./lib/fr/loglik_kep'    #program='./lib/fr/%s_%s'%(minimized_value,mod) 
    ...
    #par = np.array([1.9000000e+01, 1.4000000e+01, 5.1132310e+01, 4.0412694e+02, 1.6630000e-01, 2.1689239e+02, 2.2795899e+02, 9.0000000e+01,
     #               0.0000000e+00, 4.9177250e+01, 7.7170901e+02, 7.7260000e-02, 1.7038211e+02, 1.6805172e+02, 9.0000000e+01, 0.0000000e+00,
    #                0.0000000e+00, 6.8000000e+02, 0.0000000e+00, 1.7000000e+00])
   # bb = [np.array([ 0., 60.]), np.array([ 0., 60.]), np.array([  0., 100.]), np.array([  0., 800.]), np.array([0.   , 0.999]), np.array([-720.,  720.]), np.array([-720.,  720.]), np.array([  0., 100.]), np.array([   0., 1400.]), np.array([0.   , 0.999]), np.array([-720.,  720.]), np.array([-720.,  720.]), np.array([-130.,  130.]), np.array([-1300.,  1300.])]
...
#----------------- one more time using the Simplex method ---------------------#
    xtol = 1e-3
    for k in range(1): # run at least 3 times the minimizer
        xtol = xtol/10.0 
        print(k,xtol)
     
        result = op.minimize(nll, pp, args=(mod,par,flags, npl,vel_files, epoch, stmass, bb, gps, rtg ), method=minimzers[0], options={'xtol': xtol, 'disp': True, 'maxiter':30000, 'maxfev':30000 })
        pp = result["x"]
...
def run_mcmc_old(obj,  prior=0, samplesfile='', level=(100.0-68.3)/2.0, threads=1, doGP=False, gp_par=None, gp_kernel_id=-1, use_gp_par=[True,True,True,True], save_means=False, fileoutput=False, save_sampler=False,burning_ph=10, mcmc_ph=10, **kwargs):      
