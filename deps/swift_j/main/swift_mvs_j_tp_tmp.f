...
c                                    (real arrays)
c                 vxj,vyj,vzj   ==>  initial velocity in Jacboi coord 
c                                    (real arrays)
...
c...     next the test particles
         call step_kdk_tp_j(i1sttp,nbod,ntp,mass,j2rp2,j4rp4,
...
c                                       (real arrays)
c
c Remarks: Adopted from step_kdk_tp
c Authors:  Man Hoi Lee
c Date:    3/2/10
c Last revision: 

      subroutine step_kdk_tp_j(i1st,nbod,ntp,mass,j2rp2,j4rp4,
     &              xbeg,ybeg,zbeg,xend,yend,zend,
     &              xjt,yjt,zjt,vxjt,vyjt,vzjt,istat,dt)	
...
c...      Get the accelerations in helio frame.
     ...
     &                  xjt,yjt,zjt,istat,axjt,ayjt,azjt)
          i1st = 1    ! turn this off
      endif
