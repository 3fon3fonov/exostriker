...
ccc   The final version will be available in the Python RVMod lib.    
      ...
      integer writeflag_best_par, nt, excluded_planets(NPLMAX)
...
c      write(*,*) 'Stellar mass: '
     ...
     & 	           epoch,t0,t_max,a,ia,ma,mfit,excluded_planets)
...
c*******final output******************
 333  alamda = 0.d0
     ...
     &  nt, model_max,model_min,excluded_planets)
...
c      stop
     ...
     &               t0,t_max,ar,iar,ma,mfit,excluded_planets)  
      ...
      real*8 t(10000),y(10000),sig(10000),ys(10000),sigs(10000)
      parameter (NDSMAX=20,NPLMAX=20,MMAX=200)
      integer idsmax(NDSMAX),ts(10000),excluded_planets(NPLMAX)
...
Cc    Xianyu Tan 2011
     ...
     &           deltat,nt, model_max,model_min, excluded_planets)
     ...
     &       writeflag_best_par,writeflag_fit,excluded_planets(NPLMAX)
...
c From Numerical Recipes.
	...
	subroutine MRQMIN (x,ts,y,sig,ndata,a,ia,ma,covar,alpha,nca,
     & 	chisq,funcs,alamda,loglik,jitt,epsil,deltat,excluded_planets)

	implicit none
	integer ma,nca,ndata,ia(ma),MMAX,NDSMAX,NPLMAX,ts(ndata)
	parameter (MMAX=200,NDSMAX=20, NPLMAX=20)
	integer excluded_planets(NPLMAX)	
...
CC	  alamda = 5000.d0
     ...
     &     nca,chisq,funcs,loglik,jitt,epsil,deltat,excluded_planets)
...
c From Numerical Recipes.
	...
	subroutine MRQCOF (x,ts,y,sig,ndata,a,ia,ma,alpha,beta,nalp,
     &	 chisq,funcs,loglik,jitt,epsil,deltat,excluded_planets)

	implicit none
	integer npl,ndset,idset,ma,nalp,ndata,ia(ma),NDSMAX,MMAX,NPLMAX
	parameter (NDSMAX=20, MMAX=200, NPLMAX=20)
        integer idsmax(NDSMAX),ts(ndata), excluded_planets(NPLMAX)
...
c-----get ymod first
     ...
     &  xh,yh,zh,vxh,vyh,vzh,epsil,deltat,excluded_planets)
...
c      pause
	...
	integer nbod,ntp,nleft, excluded_planets(NPLMAX)
