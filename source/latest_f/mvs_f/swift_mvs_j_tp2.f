...
c  be integrated.
c  NOTE:  Test particles are assumed to have the first Jacobi index,
c  which works better if they orbit the star closer than the planets.
c
...
c...        Get the accelerations in Jacobi frame.
     ...
     &           xjt,yjt,zjt,sumax,sumay,sumaz,istat,
     &           axjt,ayjt,azjt)
         endif
...
c..   Drift in Jacobi coords for the full step 
      ...
      if (ntp.ne.0) then
         call drift_tp(ntp,mass(1),xjt,yjt,zjt,vxjt,vyjt,vzjt,dt,istat)
      endif
...
c...  Internals:
      ...
      real*8 eta(NPLMAX),sumaoblx,sumaobly,sumaoblz
...
c...  Now do j2 and j4 stuff
	 ...
	 do i=nbod,2,-1
	    sumaoblx = sumaoblx + mass(i)*aoblx(i)
	    sumaobly = sumaobly + mass(i)*aobly(i)
	    sumaoblz = sumaoblz + mass(i)*aoblz(i)

	    axj(i) = axj(i) + aoblx(i) + sumaoblx/eta(i-1)
	    ayj(i) = ayj(i) + aobly(i) + sumaobly/eta(i-1)
	    azj(i) = azj(i) + aoblz(i) + sumaoblz/eta(i-1)
	 enddo
      endif
...
c...  now the first terms, which are the same for all particles
      call getacch_ah0(2,nbod,mass,xh,yh,zh,ir3h,axj1,ayj1,azj1)

c...  the second terms are 0

c...  now the third terms
      call getacch_ah3_tp(nbod,ntp,mass,xh,yh,zh,xjt,yjt,zjt,
     &     istat,axj3,ayj3,azj3)
...
c...  Now do j2 and j4 stuff
     ...
     &        irjt,aoblxt,aoblyt,aoblzt)

         do i = 1,ntp
	    if(istat(i).eq.0) then
	       axjt(i) = axjt(i) + aoblxt(i)
	       ayjt(i) = ayjt(i) + aoblyt(i)
	       azjt(i) = azjt(i) + aoblzt(i)
	    endif
         enddo
      endif
