...
c Last revision: 2/22/00 MHL
      ...
      real*8 ecc2i,ecc2f,gm2,a2,e2,inc2,capom2,omega2,capm2,e2tmp
      integer ialpha2
...
c Prompt and read initial mass and radius, final mass, growth time, and
c initial and final eccentricity of first planet
     ...
     &     mass2i,rpl2i,mass2f,tgrow
      ...
      write(*,*) 'Enter initial and final ecc. of first planet :'
      read(*,*) ecc2i,ecc2f
      write(*,*) ' ecc2i,ecc2f = ',ecc2i,ecc2f
...
c Update first planet's mass and ecc. by half dt
         if(ttmp.le.tgrow) then
            mass(2) = mass2i + (mass2f - mass2i)*(ttmp/tgrow)**2*
     &                         (3.0d0 - 2.0d0*(ttmp/tgrow))

            gm2 = mass(1) + mass(2)
            call orbel_xv2el(xh(2),yh(2),zh(2),vxh(2),vyh(2),vzh(2),gm2,
     &           ialpha2,a2,e2,inc2,capom2,omega2,capm2)
            e2tmp = ecc2i + (ecc2f - ecc2i)*(ttmp/tgrow)**2*
     &                      (3.0d0 - 2.0d0*(ttmp/tgrow))
            if ((ialpha2.eq.-1).and.(e2tmp.gt.e2)) then
               e2 = e2tmp
               call orbel_el2xv(gm2,ialpha2,a2,e2,inc2,capom2,omega2,
     &              capm2,xh(2),yh(2),zh(2),vxh(2),vyh(2),vzh(2))
            endif
         else
            mass(2) = mass2f
         endif
...
c Update first planet's ecc. and mass by half dt
         if(t.le.tgrow) then
            gm2 = mass(1) + mass(2)
            call orbel_xv2el(xh(2),yh(2),zh(2),vxh(2),vyh(2),vzh(2),gm2,
     &           ialpha2,a2,e2,inc2,capom2,omega2,capm2)
            e2tmp = ecc2i + (ecc2f - ecc2i)*(t/tgrow)**2*
     &                      (3.0d0 - 2.0d0*(t/tgrow))
            if ((ialpha2.eq.-1).and.(e2tmp.gt.e2)) then
               e2 = e2tmp
               call orbel_el2xv(gm2,ialpha2,a2,e2,inc2,capom2,omega2,
