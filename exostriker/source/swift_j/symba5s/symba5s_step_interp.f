...
c Date:    11/21/96
c Last revision: 7/19/06 MHL

      subroutine symba5s_step_interp(time,iecnt,ielev,nbod,
...
c     Evolve spin axis due to torque from Sun for a half dt
      call spin_sun(nbod,nbodm,mass,xh,yh,zh,preconst,aref,sx,sy,sz,dth)

c     Evolve spin axis due to torque from other planets for a half dt
c...     Only include those guys that it is not encountering with. 
      call spin_planets_noenc(nbod,nbodm,mass,xh,yh,zh,preconst,aref,
     &     sx,sy,sz,dth,ielc,ielst)
...
c..   Do a recursion step for full dt
     ...
     &     yh,zh,vxb,vyb,vzb,dt)
      ...
      enddo
      mergecnt = 0
      call symba5s_step_recur(time,nbod,nbodm,mass,irec,ilevl,
