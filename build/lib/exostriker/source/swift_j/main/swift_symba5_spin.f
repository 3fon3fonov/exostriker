...
c
c  This version integrates the evolution of the planetary spins and
c  allows planet i's semi-major axis, eccentricity and inclination to
c  change at rates dadt(i)*a, dedt(i)*e and dedt(i)*i.
c
...
c Prompt and read precession constants, reference semimajor axes and
c initial spin axes.
      write(*,*)
     &     'Enter prec. const., aref and s :'
      do i=2,nbod
         read(*,*) preconst(i),aref(i),sx(i),sy(i),sz(i)
         write(*,*) ' i,preconst,aref,s = ',
     &              i,preconst(i),aref(i),sx(i),sy(i),sz(i)
         if (abs((sx(i)**2+sy(i)**2+sz(i)**2)-1.d0).gt.1.d-14) then
            write (*,*) ' MAIN: spin vector not normalized for i=',i
            call util_exit(1)
         endif
      enddo
...
c...    Do the initial io write
     ...
     &        xht,yht,zht,vxht,vyht,vzht,istat,outfile,iub,fopenstat)
         ...
         call io_write_spin(t0,nbod,sx,sy,sz,outfile,ius,fopenstat)
      endif
...
c Update each planet's semi-major axis, ecc. and incl. by half dt
         do i=2,nbod
            gmi = mass(1) + mass(i)
            call orbel_xv2el(xh(i),yh(i),zh(i),vxh(i),vyh(i),vzh(i),gmi,
     &           ialphai,ai,ei,inci,capomi,omegai,capmi)
            ai = ai*exp(0.5d0*dt*dadt(i))
            ei = ei*exp(0.5d0*dt*dedt(i))
            inci = inci*exp(0.5d0*dt*dedt(i))
...
c if it is time, output orb. elements, 
     ...
     &              iub,fopenstat)
               ...
               call io_write_spin(t,nbod,sx,sy,sz,outfile,ius,
     &              fopenstat)
            endif
            if(btest(iflgchk,1))  then ! bit 1 is set
